                    ...
                    <div class="user-info">
                        <div class="user-name">';
                            if ($user->getFirstName() && $user->getLastName()) {
                                echo '<h3>' . $user->getId() . '. ' . $user->getFirstName() . ' ' . $user->getLastName() . '</h3>';
                            } else {
                                echo '<h3>' . $user->getId() . '. ' . $user->getUsername() . '</h3>';
                            }
                            if ($user->getPhoneNumber()) {
                                echo '<p>' . $user->getPhoneNumber() . '</p>';
                            } else {
                                echo '<p>Brak numeru telefonu</p>';
                            }
                        echo '</div>
                    </div>
                    <div class="user-actions">
                        <a href="/edit-user/' . $user->getId() . '"><img src="../assets/images/edit.svg" alt="edit"></a>
                        <a href="/delete-user/' . $user->getId() . '"><img src="../assets/images/delete.svg" alt="delete"></a>
                    </div>
                </div>';
            }
            ?>
